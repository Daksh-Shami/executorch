...
TEST(WavLoaderTest, LoadHeaderParsesPcmMetadata) {
...
TEST(WavLoaderTest, LoadAudioData16BitNormalizesSamples) {
...
TEST(WavLoaderTest, LoadAudioData32BitNormalizesSamples) {
  const std::vector<int32_t> samples = {
      0,
...
TEST(WavLoaderTest, LoadHeaderReturnsNullWhenMagicMissing) {
...
TEST(WavLoaderTest, LoadAudioDataUnsupportedBitsTriggersCheck) {
  // 24-bit PCM is unsupported and should trigger ET_CHECK_MSG in the loader.
  const std::vector<uint8_t> wav_bytes = make_pcm_wav_bytes(24, {0, 1, -1});
  TempFile file(wav_bytes.data(), wav_bytes.size());

  EXPECT_DEATH(load_wav_audio_data(file.path()), "Unsupported bits per sample");
}
